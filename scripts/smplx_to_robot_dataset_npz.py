...
def check_memory(threshold_gb=30):  # adjust based on your available memory
    ...
    if available_memory_gb < threshold_gb:
        print(
            f"[WARNING] Memory usage:{used_memory_gb:.2f} GB, available:{available_memory_gb:.2f} GB, exceeding the threshold of {threshold_gb} GB.")
        return True
    return False
...
def process_file(smplx_file_path, tgt_file_path, tgt_robot, SMPLX_FOLDER, tgt_folder, total_files, verbose=False):
    ...
    tgt_fps = 30
    try:
        smplx_frame_data_list, aligned_fps = get_smplx_data_offline_fast(smplx_data, body_model, smplx_output,
                                                                         tgt_fps=tgt_fps)
    except Exception as e:
    ...
    motion_data = {
        ...
        "link_body_list": np.array(body_names),  # 转换为numpy array以便保存
    }

    os.makedirs(os.path.dirname(tgt_file_path), exist_ok=True)

    # 使用 np.savez_compressed 替代 pickle.dump
    np.savez_compressed(tgt_file_path, **motion_data)

    # Progress print based on tgt_folder (counting .npz files)
    done = 0
...
def main():
    ...
    parser.add_argument("--no_filter", default=False, action="store_true",
                        help="Do not exclude hard/infeasible motions.")
    ...
    # collect hard motion lists; be tolerant to filename differences
    hard_motions = []
    exclude_file_content = ["BMLrub", "EKUT", "crawl", "_lie", "upstairs", "downstairs"]
    if not args.no_filter:
        hard_motion_filenames = ["0.txt", "1.md.txt", "1.txt"]
        hard_motions_paths = []
        for filename in hard_motion_filenames:
            path = hard_motions_folder / filename
            if path.exists():
                hard_motions_paths.append(path)
            else:
                print(f"[WARNING] Hard motion list not found: {path}")
    ...
    for dirpath, _, filenames in os.walk(src_folder):
        for filename in natsorted(filenames):
            ...
            if filename.endswith((".pkl", ".npz")):
                ...
                tgt_file_path = os.path.splitext(tgt_file_path)[0] + ".npz"
